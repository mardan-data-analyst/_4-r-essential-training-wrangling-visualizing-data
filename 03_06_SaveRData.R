...
df <- import("/cloud/project/exercise-files/data/StateData.xlsx") %>%
  as_tibble() %>%
  select(state_code, 
    psychRegions,
...
# Use save()
save(df, file = "/cloud/project/exercise-files/data/StateData.rdata")
...
load("/cloud/project/exercise-files/data/StateData.rdata")
df  # Check data
...
import("/cloud/project/exercise-files/data/StateData.rdata") %>%
  print()

# import() can rename file as new object and print
new_df <- import("/cloud/project/exercise-files/data/StateData.rdata") %>%
  print()
...
# Use saveRDS()
df %>% saveRDS("/cloud/project/exercise-files/data/StateData.rds")

# Or use write_rds() from tidyverse. Not compressed by 
# default, as "space is generally cheaper than time."
df %>% write_rds("/cloud/project/exercise-files/data/StateData2.rds")
...
import("/cloud/project/exercise-files/data/StateData.rds")

# With a destintation object, import saves data
new_df <- import("/cloud/project/exercise-files/data/StateData.rds") %>%
  print()
