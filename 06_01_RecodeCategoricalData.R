...
df <- import("/cloud/project/exercise-files/data/MobileOS_US.xlsx") %>%
  as_tibble() %>%
  print()
...
df %<>% 
  ...
  dplyr::select(MobileOS, OS_2010) %>%
  print()

# Check for outliers
df %>% 
  dplyr::select(OS_2010) %>% 
  boxplot(horizontal = T)

## Convert to integer otherwise next code results in error
df %<>% 
