...
## USE package name:function name for example, dplyr:select()
## otherwise the code results in error

df %>%
  group_by_key(key) %>%                          # Group index
  index_by(~ max(df$index)) %>%               # Group date
  summarize(ROI = max(value)/min(value)) %>%  # Find ROI
	dplyr::select(Index = key, ROI) %>%                # Select data
