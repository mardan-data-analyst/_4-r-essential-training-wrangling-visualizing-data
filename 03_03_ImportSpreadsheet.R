...
(df1 <- read_csv("/cloud/project/exercise-files/data/StateData.csv"))


# Import other formats with rio::import() from rio
(df2 <- import("/cloud/project/exercise-files/data/StateData.xlsx") %>% as_tibble())

# Or combine import with data preparation
df3 <- import("/cloud/project/exercise-files/data/StateData.xlsx") %>%
  as_tibble() %>%
  select(state_code, 
    psychRegions,
